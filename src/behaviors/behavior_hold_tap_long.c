 ...
 *
 * Copy of ZMK's behavior_hold_tap.c with an added long-hold stage: when the
 * hold was decided by the timer and no other key is pressed before
 * long-hold-ms, the hold binding is released and the long-hold binding pressed.
 *
 * SPDX-License-Identifier: MIT
 */

#define DT_DRV_COMPAT zmk_behavior_hold_tap_long
...
#define ZMK_BHV_HOLD_TAP_MAX_HELD CONFIG_ZMK_BEHAVIOR_HOLD_TAP_LONG_MAX_HELD
#define ZMK_BHV_HOLD_TAP_MAX_CAPTURED_EVENTS CONFIG_ZMK_BEHAVIOR_HOLD_TAP_LONG_MAX_CAPTURED_EVENTS
...
struct active_hold_tap {
    ...
    bool long_work_is_cancelled;
    // set once any other key is pressed while this key is held
    bool long_hold_blocked;
...
const struct zmk_listener zmk_listener_behavior_hold_tap_long;
...
static void release_captured_events() {
    ...
    for (int i = 0; i < ZMK_BHV_HOLD_TAP_MAX_CAPTURED_EVENTS; i++) {
        ...
        case ET_CODE_CHANGED:
            ...
            ZMK_EVENT_RAISE_AT(captured_event->data.keycode, behavior_hold_tap_long);
            break;
        case ET_POS_CHANGED:
            ...
            ZMK_EVENT_RAISE_AT(captured_event->data.position, behavior_hold_tap_long);
            break;
        default:
...
static struct active_hold_tap *store_hold_tap(struct zmk_behavior_binding_event *event,
    ...
    for (int i = 0; i < ZMK_BHV_HOLD_TAP_MAX_HELD; i++) {
        ...
        active_hold_taps[i].long_hold_blocked = false;
        return &active_hold_taps[i];
    }
    return NULL;
}
...
static void clear_hold_tap(struct active_hold_tap *hold_tap) {
    ...
    hold_tap->long_work_is_cancelled = false;
    hold_tap->long_hold_blocked = false;
}
...
static int press_long_hold_binding(struct active_hold_tap *hold_tap) {
    struct zmk_behavior_binding_event event = {
        .position = hold_tap->position,
        .timestamp = hold_tap->timestamp,
#if IS_ENABLED(CONFIG_ZMK_SPLIT)
    ...
    struct zmk_behavior_binding binding = {
        .behavior_dev = hold_tap->config->long_hold_behavior_dev, .param1 = hold_tap->param_tap};
    store_last_hold_tapped(hold_tap);
    return zmk_behavior_invoke_binding(&binding, event, true);
}

static int release_long_hold_binding(struct active_hold_tap *hold_tap) {
    struct zmk_behavior_binding_event event = {
        .position = hold_tap->position,
        .timestamp = hold_tap->timestamp,
#if IS_ENABLED(CONFIG_ZMK_SPLIT)
...
static int release_binding(struct active_hold_tap *hold_tap) {
    if (hold_tap->status == STATUS_LONG_HOLD) {
        return release_long_hold_binding(hold_tap);
    }
...
// Only a hold decided by the timer, with no other key pressed since, can become a long hold.
static bool can_long_hold(struct active_hold_tap *hold_tap) {
    return hold_tap->status == STATUS_HOLD_TIMER && !hold_tap->long_hold_blocked;
}

static void schedule_long_hold(struct active_hold_tap *hold_tap) {
    if (!can_long_hold(hold_tap)) {
        return;
    }

    int32_t long_hold_ms_left = (hold_tap->timestamp + hold_tap->config->long_hold_ms) - k_uptime_get();
    k_work_schedule(&hold_tap->long_work, K_MSEC(MAX(long_hold_ms_left, 0)));
}

static void decide_long_hold(struct active_hold_tap *hold_tap) {
    if (!can_long_hold(hold_tap)) {
        return;
    }

    LOG_DBG("%d decided long-hold", hold_tap->position);
    // with retro-tap the hold binding is not pressed on a timer decision
    if (!hold_tap->config->retro_tap || hold_tap->config->hold_while_undecided) {
        release_hold_binding(hold_tap);
    }
    hold_tap->status = STATUS_LONG_HOLD;
    press_long_hold_binding(hold_tap);
}

static void block_long_holds(uint32_t pressed_position) {
    for (int i = 0; i < ZMK_BHV_HOLD_TAP_MAX_HELD; i++) {
        struct active_hold_tap *hold_tap = &active_hold_taps[i];
        if (hold_tap->position == pressed_position ||
            hold_tap->position == ZMK_BHV_HOLD_TAP_POSITION_NOT_USED ||
            hold_tap->long_hold_blocked) {
            continue;
        }
        LOG_DBG("%d long-hold blocked by key %d", hold_tap->position, pressed_position);
        hold_tap->long_hold_blocked = true;
        k_work_cancel_delayable(&hold_tap->long_work);
    }
}
...
static int on_hold_tap_binding_released(struct zmk_behavior_binding *binding,
    ...
    // Same for the long-hold timer.
    if (event.timestamp > (hold_tap->timestamp + hold_tap->config->long_hold_ms)) {
        decide_long_hold(hold_tap);
    }
    int long_work_cancel_result = k_work_cancel_delayable(&hold_tap->long_work);

    decide_hold_tap(hold_tap, HT_KEY_UP);
    decide_retro_tap(hold_tap);
    bool was_long_hold = hold_tap->status == STATUS_LONG_HOLD;
    release_binding(hold_tap);
    ...
    if (work_cancel_result == -EINPROGRESS || long_work_cancel_result == -EINPROGRESS) {
        // let the timer handlers clean up
        // if we'd clear now, a timer may call back for an uninitialized active_hold_tap.
        LOG_DBG("%d hold-tap timer work in event queue", event.position);
        hold_tap->work_is_cancelled = work_cancel_result == -EINPROGRESS;
        hold_tap->long_work_is_cancelled = long_work_cancel_result == -EINPROGRESS;
    } else {
...
static int behavior_hold_tap_long_listener(const zmk_event_t *eh) {
...
ZMK_LISTENER(behavior_hold_tap_long, behavior_hold_tap_long_listener);
ZMK_SUBSCRIPTION(behavior_hold_tap_long, zmk_position_state_changed);
// this should be modifiers_state_changed, but unfrotunately that's not implemented yet.
ZMK_SUBSCRIPTION(behavior_hold_tap_long, zmk_keycode_state_changed);

static void behavior_hold_tap_timer_work_handler(struct k_work *item) {
    struct k_work_delayable *d_work = k_work_delayable_from_work(item);
    struct active_hold_tap *hold_tap = CONTAINER_OF(d_work, struct active_hold_tap, work);

    if (hold_tap->work_is_cancelled) {
        hold_tap->work_is_cancelled = false;
        if (!hold_tap->long_work_is_cancelled) {
            clear_hold_tap(hold_tap);
        }
    } else {
        decide_hold_tap(hold_tap, HT_TIMER_EVENT);
...
static void behavior_hold_tap_long_work_handler(struct k_work *item) {
    struct k_work_delayable *d_work = k_work_delayable_from_work(item);
    struct active_hold_tap *hold_tap = CONTAINER_OF(d_work, struct active_hold_tap, long_work);
...
static int behavior_hold_tap_init(const struct device *dev) {
    ...
    if (init_first_run) {
        for (int i = 0; i < ZMK_BHV_HOLD_TAP_MAX_HELD; i++) {
            ...
            k_work_init_delayable(&active_hold_taps[i].long_work,
                                  behavior_hold_tap_long_work_handler);
...
#define KP_INST(n)                                                                                 \
    static const struct behavior_hold_tap_config behavior_hold_tap_config_##n = {                  \
        ...
        .long_hold_behavior_dev = DEVICE_DT_NAME(DT_INST_PHANDLE_BY_IDX(n, bindings, 2)),          \
